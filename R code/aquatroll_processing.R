...
# dropout_aquatroll <- function(input, ignore.marked=T){
...
#   lat <- mean(D[,grep("latitude",columns,ignore.case = T)],na.rm=T)
#   lon <- mean(D[,grep("longitude",columns,ignore.case = T)],na.rm=T)
#   ### max depth
#   z_max <- max(D[,grep('depth',columns,ignore.case = T)],na.rm=T)
#   ### is there data dropout
#   if(ignore.marked==T){
#     D <- D[,-grep('marked',columns,ignore.case = T)] 
#   }
#   missingness <- unlist(lapply(D,function(x) sum(is.na(x))))
#   n_samp <- nrow(D)
#   ### data ranges
#   # dat_range <- lapply(D,range,na.rm=T)
#   
#   ### save output
...
#                     lon_dd=lon,
#                     lat_dd=lat,
#                     dropout=toString(columns[which(missingness>0)]),
...
process_aquatroll <- function(input, lat=NA, lon=NA, resolution=1, z_min=2, span=5, write_csv=T, set_wd=NA, plot=T){
  ...
  ### Aquatroll serial number
  row_h <- which(H[,1] == "Device Model = Aqua TROLL 600 Vented")
  ### depreciated due to error
  # aquatroll_sn <- as.numeric(strsplit(D[row_h+1,1],'=')[[1]][2])
  ### fixed error 2021/07/30
  aquatroll_sn <- as.numeric(strsplit(as.character(H[row_h+1,1]),'=')[[1]][2])
  ...
  ### older code depreciated; 2021/08/02
  # breaks <- seq(0,ceiling(max(D$depth_m)),resolution)
  # temp_l <- approx(D$depth_m,D$temp_c,xout=breaks,ties=mean)
  # sal_l <- approx(D$depth_m,D$sal_ppt,xout=breaks,ties=mean)
  # chl_l <- approx(D$depth_m,D$chl_ugl,xout=breaks,ties=mean)
  # do_l <- approx(D$depth_m,D$oxy_mgl,xout=breaks,ties=mean)
  # 
  # data_interp <- data.frame(dtime_utc=paste(with_tz(D$dtime_utc[1],tz='UTC'),'UTC',sep=' '),
  #                           lat_dd=D$lat_dd[1],
  #                           lon_dd=D$lon_dd[1],
  #                           depth_m=breaks,
  #                           temp_c=temp_l$y,
  ...
  if(write_csv){
    ...
    # write.csv(D,paste('processed_',D$dtime_utc[1],'.csv',sep=''),row.names = F)
    # write.csv(data_interp,paste('interp_',D$dtime_utc[1],'.csv',sep=''),row.names = F)
...
process_htm <- function(htm, lat=NA, lon=NA, resolution=1, z_min=2, span=5, write_csv=T, set_wd=NA, plot=T){
  ...
  ### Aquatroll serial number
  row_h <- which(D[,1] == "Device Model = Aqua TROLL 600 Vented")
  ### depreciated due to error
  # aquatroll_sn <- as.numeric(strsplit(D[row_h+1,1],'=')[[1]][2])
  ### fixed error 2021/07/30
  aquatroll_sn <- as.numeric(strsplit(as.character(D[row_h+1,1]),'=')[[1]][2])
  
  # get header row of data
  row_d <- which(D[,1] == "Date Time")
  ind_aquatroll <- grep(aquatroll_sn,D[row_d,])
  ind_lat <- grep('Latitude',D[row_d,])
  ind_lon <- grep('Longitude',D[row_d,])
  ind_depth <- grep('Depth',D[row_d,])
  ind_sal <- grep('Salinity',D[row_d,])
  ind_temp <- grep('Temperature',D[row_d,]) %>%
    intersect(ind_aquatroll)
  ind_chl <- grep('Chlorophyll-a Concentration',D[row_d,])
  ind_chlf <- grep('Chlorophyll-a Fluorescence',D[row_d,])
  ind_oxy <- grep('RDO Concentration',D[row_d,])
  ind_cond <- grep('Actual Conductivity',D[row_d,])
  ind_cond2 <- grep('Specific Conductivity',D[row_d,])
  ind_all <- c(1,ind_lat,ind_lon,ind_depth,ind_sal,ind_temp,ind_chl,ind_chlf,ind_oxy,ind_cond,ind_cond2)
  # extract data
  D <- D[(row_d):nrow(D),]
  if(nrow(D)<5){
  ...
  ### strip serial numbers
  names(D) <- gsub(" \\([0-9]+\\)", "", D[1,])
  ### what are depth units?
  depth_unit <- substr(names(D)[ind_depth],7,20)
  ### keep only variables of interest
  D <- data.frame(D[-1,ind_all])
  D[,2:ncol(D)] <- lapply(D[,2:ncol(D)],as.numeric) ### convert from character to numeric
  ### datetime
  D$Date.Time <- with_tz(force_tz(ymd_hms(D$Date.Time),tz='America/New_York'),'UTC')
  dtime <- D$Date.Time[1] ### start time
  ...
  ### if you know lat/lon is missing
  if(!is.na(lat) | !is.na(lon)){
    warning(paste("\n\nUser supplied fields added: ",paste(flds_miss, collapse=', '),"\n\n"))
    D$lat_dd <- lat
    D$lon_dd <- lon
  }
  ### if lat/lon is missing, interactively add them in
  flds_miss <- setdiff(re_names, names(D))
  if (length(flds_miss) > 0){
    warning(paste("\n\nAdding fields: ",paste(flds_miss, collapse=', '),"\n\n"))
    lat_i <- readline(prompt = 'Enter latitude (decimal degrees): ')
    lon_i <- readline(prompt = 'Enter longitude (decimal degrees): ')
    D$lat_dd <- as.numeric(lat_i)
    D$lon_dd <- as.numeric(lon_i)
  ...
  ### add htm filename for reference
  D$htm <- htm
  ...
  data_interp <- data.frame(htm=htm,
                            dtime_utc=dtime,
                            lat_dd=D$lat_dd[1],
                            lon_dd=D$lon_dd[1],
                            depth_m=breaks,
...
#   ind_chlf <- grep('Chlorophyll-a Fluorescence',columns)
#   ind_oxy <- grep('RDO Concentration',columns)
#   ind_all <- c(1,ind_lat,ind_lon,ind_depth,ind_sal,ind_temp,ind_chl,ind_chlf,ind_oxy)
#   ### keep only variables of interest
#   D <- data.frame(D[,ind_all])
#   D[,2:ncol(D)] <- lapply(D[,2:ncol(D)],as.numeric) ### convert from character to numeric
#   ### datetime
...
#   }
#   ### required fields
#   flds_req <- c("Date.Time", "Latitude....", "Longitude....", "Depth..m.", "Salinity..ppt.", "Temperature...C.", "Chlorophyll.a.Concentration..µg.L.", "Chlorophyll.a.Fluorescence..RFU.", "RDO.Concentration..mg.L.")
#   re_names <- c('dtime_utc','lat_dd','lon_dd','depth_m','sal_ppt','temp_c','chl_ugl','chl_rfu','oxy_mgl')
#   ### reorder columns to be consistent
#   D <- D[,order(names(D))]
#   reorder <- order(flds_req)
#   flds_req <- flds_req[reorder]
...
#   D <- D[order(D$dtime_utc),]
#   # total duration
#   duration_total <- as.numeric(D$dtime_utc[nrow(D)]-D$dtime_utc[1],units='secs')
#   # filter for downcast (not up)
#   row_end <- which.max(D$depth_m)
#   D <- D[1:row_end,]
#   # down duration
#   duration_down <- as.numeric(D$dtime_utc[nrow(D)]-D$dtime_utc[1],units='secs')
#   # max depth of cast
#   z_max <- max(D$depth_m,na.rm=T)
#   # number of samples
#   n_samp <- nrow(D)
#   # mean descent rate
#   dz_dt <- mean(abs(diff(D$depth_m)/as.numeric(diff(D$dtime_utc))),na.rm=T)
#   ### save output
#   out <- data.frame(input=input,
#                     time_utc=dtime,
#                     aquatroll_sn=aquatroll_sn,
...
# summary_aquatroll <- function(input){
#   if(file_ext(input)!='csv' & file_ext(input)!='htm'){
#     warning(paste('\n\n File format needs to be csv or htm! \n\n'),
#             immediate. = T)
#   }
...
summary_htm <- function(htm){
  ...
  aquatroll_sn <- as.numeric(strsplit(as.character(D[row_h+1,1]),'=')[[1]][2])
  ### bluetooth serial number
  row_h2 <- which(D[,1] == "Device Model = In-Situ Bluetooth Device")
  handheld_sn <- as.numeric(strsplit(as.character(D[row_h2+1,1]),'=')[[1]][2])
  # get header row of data
  row_d <- which(D[,1] == "Date Time")
  ind_aquatroll <- grep(aquatroll_sn,D[row_d,])
  ind_lat <- grep('Latitude',D[row_d,])
  ...
  depth_unit <- substr(names(D)[ind_depth],7,20)
  ### keep only variables of interest
  # D <- D[-1,ind_all] ### tibble column names are more natural
  D <- data.frame(D[-1,ind_all])
  D[,2:ncol(D)] <- lapply(D[,2:ncol(D)],as.numeric) ### convert from character to numeric
  ### datetime
  D$Date.Time <- with_tz(force_tz(ymd_hms(D$Date.Time),tz='America/New_York'),'UTC')
  ...
  D <- D[order(D$dtime_utc),]
  # length of cast
  duration <- D$dtime_utc[nrow(D)]-D$dtime_utc[1]
  # max depth of cast
  z_max <- max(D$depth_m,na.rm=T)
  # number of samples
  n_samp <- nrow(D)
